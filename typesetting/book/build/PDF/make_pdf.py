...
typ-v1.2.0 修订（同母书 v1.2.0 机制移植，2026-09-03 规范 10/11 章对齐）：
① 整档缩放根治：Chromium page.pdf() 对 print 布局中「内容超出内容盒(~672px@96dpi)」
的元素整本等比压缩（母书 OCEAN 五维 flow-tree 超宽行 ~866px → ×0.892，正文 12pt→
10.7pt）。修复 = 消除触发源：print 态 border-box 全局 / body 收 665px 左对齐 /
表格 width:100%+单元格断行 / 流程与 edge 长 token 标签 pre→normal 断行 / 树形图
.flow-treegroup 按需 zoom（预算 665，print 媒体下测量）。修复后正文回到自然 12pt。
② 目录页码列修订（同母书）：.toc 容器装饰右边框 x=543.4，三位数页码右缘 x=548 穿框
4.6pt。页码右对齐基线 548→538 / 虚线终点 532→521 / link_rect 右缘 556→543。
③ 阈值随自然字号重校准：add_toc_dots 匹配 >=12→>=15；find_h1_page >15 保持。
④ 目录行跨行合并修复：超长目录标题折行后碎片（单字）曾独立命中正文标题，把顺序锚
last_target 抬高致真书签漏配。现按 y 近邻(<23pt)合并跨行续行、纯数字页码行剔除、
顺序锚定降级为首选起点 + miss 后全局回退兜底。
...
@media print {
  /* ===== 整档缩放根治（typ-v1.2.0 移植，同母书 v1.2.0 实证校准机制）=====
...
def render_pdf():
    ...
    with sync_playwright() as p:
        ...
        # 关键：后续测量/zoom 一律切到 print 媒体（typ-v1.2.0 移植，同母书 v1.2.0）——
        ...
        # 树形图按需 zoom（typ-v1.2.0 移植，同母书 v1.2.0）：Chromium page.pdf()
        # 对「print 布局内容超内容盒(~672px@96dpi)」整本等比压缩。PRINT_CSS 已
        # 消除软性触发源（body 收 665/表格与 code 断行/流程标签 pre→normal），
        # 此处只处理硬溢出：等宽 nowrap 树行。预算 = 665（内容盒），并减去树
        # 左偏移——树右缘 ≤665 才不触发缩放。zoom 已在 print 媒体下执行，测量
        # 与 page.pdf() 布局一致。
...
def add_toc_dots(path, h1_page):
    ...
    # typ-v1.2.0 修订：① 先按页收集全部行并按 y 排序；② 跨行标题合并——
    ...
    # typ-v1.2.0 修订；此前缩放态正文 10.7pt 用 >=12 即可，现随字号重校准——
    # 12pt 正文会被 >=12 误命中为标题，须抬到 >=15）
    # 顺序锚定降级（typ-v1.2.0 修订）：last_target 只作首选起点——顺序命中失败后
