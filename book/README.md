...
| **入书规范（必读）** | 仓库 `typesetting/STRUCTURE_GUIDE.md`——转化入书排版规范（转化规则 + 配套同步清单 + 字数/版本号全量落点 + 构建管线 + PDF 打印版式） |
...
| 第 III 部 边界 | `part3_边界/` | 09、09a~09f | 这套规则能走多远（穷举的宿命） |
| 第 IV 部 重估 | `part4_重估/` | 10、10a、10b | 让渡之后人还剩什么（认知重估、伦理牢笼与语言的天花板） |

全书约 16.2 万字，35 章（版权页 → 序言 → 阅读指南 → 四部 → 结语 → 附录）。字数口径：汉字+英文单词+数字串（出版折算，中英混杂标准算法）。
