...
十一篇编号文章 + 一篇操作层 + 两篇注脚（检索层 09a + 推动层 09b），一个闭环、一种架构范式：
...
**01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 08a → 09 → 09a → 09b**（09 为边界篇，09a 为 09 的检索层注脚，09b 为 09 的推动层注脚，依赖关系与定位见 COGITO_README）；**10 → 10a 为认知重估篇**（跨全系列的元审视：10 讲让渡与力气活终结，10a 讲让渡之后必须画圈——伦理牢笼。可通读全系列后阅读，也可独立阅读）
...
- **09 系列容器约定**：穷举宿命与稳定-随机悖论主题的沉淀进入 09 系列（09 + 09a + 09b + 未来同主题扩展）——09 为边界篇（横跨重型技能 01-07 与智能体协作 08/08a 的元审视），09a 为检索层注脚（RAG is_header 探测实战实证），09b 为推动层注脚（配置推动的穷举一致性实证）
